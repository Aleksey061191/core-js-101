...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (top1 < rect2.top || top2 < rect1.top || width1 < rect2.left || width2 < rect1.left) {
    return false;
  }
  return true;
}
...
function isCreditCardNumber(ccn) {
  ...
  if (arr.length % 2 === 0) {
    arr = arr.map((item, index) => (index % 2 === 0 ? +item * 2 : +item));
  } else {
    arr = arr.map((item, index) => (index % 2 !== 0 ? +item * 2 : +item));
  }
  arr = arr.map((item) => (item > 9 ? item - 9 : item));
...
function getCommonDirectoryPath(pathes) {
  ...
  const newArr = [];
  Object.keys(obj).map((item) => (obj[item] === arr.length ? newArr.push(item) : ''));
